...
- Automated quality gates that block incomplete, weak, or under-sourced output.
...
| EasyClaw required code ZIP | `autodirector-code.zip` | `npm run zip:code` rebuilds a clean source ZIP. It includes the runnable local app, Agent skills, examples, server code, plugin code, and verification scripts while excluding `intro-site/`, public media, screenshots, local state, dependency folders, build output, generated packages, nested ZIPs, logs, and macOS metadata. |
| Public read-only control UI must be reachable | `intro-site/control-ui/` served at `/control-ui/` | Public browser audit found all four views reachable, no `/api/` links, disabled message entry, no old demo copy, and no horizontal overflow. |
...
| Public control UI project label should stay compact | `src/App.tsx`, `intro-site/control-ui/` | The public read-only run title is `交付样片 v10`, and healthcheck rejects the old long title. Project rows include explicit labels for title, runtime, and status. |
| Control UI shell labels should be Chinese and calm | `src/App.tsx`, `scripts/healthcheck.mjs` | Bottom console, setup dialog, onboarding actions, blocked-render notices, copy states, and handoff cards now use Chinese UI labels. Healthcheck requires `流水线时间轴` and `工具控制台`, and rejects stale English shell labels. |
| Quality operations should read as product behavior | `src/App.tsx`, `src/data/autodirector.ts`, `server/index.mjs`, `intro-site/pipeline.*`, `scripts/smoke.mjs` | UI labels use `Quality Gate`, `自动质检`, `质量门`, and `自动交付`; visible copy avoids legacy audit/review language. |
| Visible team terminology should consistently say Agent | `README.md`, `DESIGN.md`, `plugins/autodirector-codex/*`, `docs/agent-skills/*`, `server/index.mjs`, `server/codex-native-agents.mjs` | User-facing docs, plugin skills, generated package text, server prompts, logs, and MCP descriptions use Agent terminology. Internal compatibility names do not surface in the UI. |
...
| Agents are persistent native Codex threads | `server/codex-native-agents.mjs` creates per-run/per-agent threads and records `run.codexThreads[agentId]`. |
| No `codex exec` Agent path | Static checks no longer find the retired CLI shortcut. Production waits on native artifact submissions instead of shelling out to a fabricated completion path. |
...
| Quality gates can block weak output | Smoke coverage confirms the quality/imagegen gate blocks publishing `final.mp4` when required image assets are absent. |
...
| Source ZIP is clean | `scripts/package-code.mjs` creates root `autodirector-code.zip` while excluding `intro-site/`, public media, built assets, `.autodirector`, Playwright output, logs, Python bytecode/cache, legacy audit-style entry names, and generated ZIP copies; it fails the build if forbidden entries are present after packaging. |
...
rg -n "9 个|8 个核心|Eight focused|Script Agent|Runtime Agent" README.md src server docs plugins intro-site -g '!node_modules'
...
- `npm run smoke:codex` covers three native/OAuth Agent behaviors: waiting for real artifact submission, completing a deterministic Producer -> Research -> Director -> Asset -> Programmer -> Render -> Quality artifact chain, and blocking publication when required image assets are missing.
- `npm run healthcheck:public` checks the current public v10 delivery film/package: `720x1280`, `31.1s`, `video/mp4`, and `application/zip`; it also enforces CSS cache-busting, localized static navigation, manifest-declared control UI deep links, polished copy gates, UI style guardrails, read-only bundle scans, public ZIP cleanliness, and clean ZIP entry names.
- `npm run build:public-ui` produces the current read-only control UI assets; the public `/control-ui/` page serves that static bundle.
...
- Public static pages and public read-only control UI were checked at mobile and desktop widths with no horizontal overflow, no unlabeled controls, no enabled public mutation controls, no computed negative letter-spacing on audited headings/controls, and no stale audit copy. The latest browser pass covered all public control UI views and confirmed no `/api/` links and no editable public textarea.
